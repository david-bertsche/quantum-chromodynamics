   ...
   // Macro to calculate the QCD background using the data driven ABCD method
   
   // To Compile and Run:  $ root -l Do_QCD_W.C++
   // To Run:              $ root -l Do_QCD_W.C
...
void Do_QCD_W(){
   ...
   try{
      ...
      //muon
      //input data file
      TFile df("out_data1.root");
      //input mc file
      TFile mcf("mc_all.root");
      //create output file
      TFile *ntup = new TFile("Output/ABCD_W.root","RECREATE");

//      //electron 
//      TFile df("/afs/cern.ch/user/d/dbertsch/work/Jpsi/Code/W_ele/Output/out_data_ele.root");
//      TFile mcf("/afs/cern.ch/user/d/dbertsch/work/Jpsi/Code/W_ele/Output/mc_all.root");
//      TFile *ntup = new TFile("Output/ABCD_W_ele.root","RECREATE");
      ...
      TH1D *data_m  = (TH1D*)df.Get("h32")->Clone("Data_Wmt_m");

      //QCD background, which comes from data driven ABCD method, must be calculated
      TH1D *ABp = new TH1D("ABp","Binned fake-factors A/B #mu+",40,0,200);
      TH1D *ABm = new TH1D("ABm","Binned fake-factors A/B #mu-",40,0,200);
      ...
      // plus: 1 = all events, 25 = bin 25 ... 195 = bin 195
      // minus: 1001 = all events, 1025 = bin 25 ... 1195 = bin 195
